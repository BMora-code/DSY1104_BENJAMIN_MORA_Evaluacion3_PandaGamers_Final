...
const Header = () => {
  ...
  // Mostrar nombre legible del usuario: preferir `name`, luego `nombre`, luego username, luego extraer local-part del email
  const getDisplayName = (u) => {
    if (!u) return null;
    const raw = u.name || u.nombre || u.username || u.email || null;
    if (!raw) return 'Usuario';
    // si raw es un email, tomar la parte antes de @
  ...
  // Determina si el usuario tiene rol admin (acepta 'admin', 'ADMIN', 'role_admin', 'ROLE_ADMIN', etc.)
  const isAdmin = (u) => {
    if (!u || !u.role) return false;
    const r = String(u.role).toLowerCase();
    return r === 'admin' || r === 'role_admin' || r.endsWith('_admin') || r.includes('admin');
  };
