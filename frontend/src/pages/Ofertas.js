  ...
  useEffect(() => {
    const loadOfertas = async () => {
      try {
        ...
        console.log('Productos obtenidos del backend:', allProducts.length);

        // Obtener ofertas desde el backend (endpoint público)
        const respOffers = await offersAPI.getPublic();
        const ofertasAdmin = Array.isArray(respOffers.data) ? respOffers.data : (respOffers.data || []);
        console.log('Ofertas obtenidas del backend:', ofertasAdmin);

        // Mapear ofertas con productos del backend (allProducts ya viene normalizado por productsAPI)
        const productosConOferta = ofertasAdmin.map(oferta => {
          ...
          if (producto) {
            return {
              ...
              price: oferta.price || Math.round(producto.price * (1 - (oferta.discount || 0) / 100)),
            ...
            return {
              ...
              price: oferta.price,
              originalPrice: oferta.originalPrice || (oferta.price ? Math.round(oferta.price * (100 / (100 - (oferta.discount || 0)))) : 0),
              discount: oferta.discount,
              category: 'Error',
              image: 'https://via.placeholder.com/300x200/dc3545/ffffff?text=Producto+No+Encontrado',
              stock: 0,
              ofertaId: oferta.id
            };
          }
        ...
        // Fallback a dataStore local si el backend no responde
        const ofertasAdmin = dataStore.getOfertas();
        const allProducts = dataStore.getProducts();
        const productosConOferta = ofertasAdmin.map(oferta => {
          const producto = allProducts.find(p => String(p.id) === String(oferta.productId));
          return producto ? { ...producto, ...oferta } : null;
        }).filter(Boolean);
        setOfertas(productosConOferta);
        setFilteredOfertas(productosConOferta);
        setLoading(false);
      }
    };
